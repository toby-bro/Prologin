...
def stabilite_maximale(n: int, k: int, p: int, accroches: List[int]) -> None:
    ...
    for nb_stab in range(1,k+1):
        ...
        if best_tableau == []:
            ...
            tableau, stab = best_tableau, best_stab
            indice_ajout, stab_ajout = best_stab_individuel(accr_libres, p)
            tableau.append(accr_libres[indice_ajout:indice_ajout+4])
            accr_libres = accr_libres[:indice_ajout] + accr_libres[indice_ajout+4:]
            stab += stab_ajout
            tab_bas, accr_libres_bas = init_par_le_bas(nb_stab, accroches)
            stab_bas = 0
            for i in tab_bas:
        ...
        # accr libre n'est pas modifié par la manip donc ...
...
if __name__ == "__main__":
    ...
    # quand je rajoute un point il faut que je verifie si en decalant tout ceux auquel il est collé c'est pas mieux (s'il se colle à gauche par exemple)
    
    # 11
    # 3
    # 100
    # 3 1 4 4 5 9 2 6 7 4 -5 ne marche pas avec aucun de mes programmes...